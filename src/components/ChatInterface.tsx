...
export default function ChatInterface({ mode = "popup" }: ChatInterfaceProps) {
  ...
  const handleExecute = useCallback(
    async (overrideInput?: string, overridePromptId?: string) => {
      ...
      if (!currentTabId || !settings?.selectedModelId || !promptIdToUse) {
        console.warn(`[${mode}] Missing required data:`, {
          currentTabId,
          selectedModelId: settings?.selectedModelId,
          selectedPrompt: promptIdToUse,
        });
        return;
      }
      ...
      console.debug(`[${mode}] Selected prompt:`, {
        id: prompt.id,
        name: prompt.name,
        text: prompt.text,
        type: prompt.type,
      ...
      if (prompt.type === PromptType.WITH_WEBPAGE) {
        console.debug(
          `[${mode}] Requesting page scrape from tab:`,
          currentTabId,
        );
        try {
  ...
  if (!session || !settings)
    return (
      <div class="p-4 text-slate-500 dark:text-slate-400 font-medium bg-white dark:bg-slate-900">
        Loading...
      </div>
    );
  ...
  useEffect(() => {
    ...
    if (
      !settings.selectedModelId ||
      !models.some(
        (m) => `${m.providerId}:${m.id}` === settings.selectedModelId,
      )
    ) {
      setSettings({
        ...settings,
  ...
  useEffect(() => {
    ...
    if (
      settings.lastSelectedPromptId &&
      prompts.some((p) => p.id === settings.lastSelectedPromptId)
    ) {
    ...
    <div class={containerClass}>
      ...
      <header class="flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shrink-0">
        <div class="flex items-center gap-2">
          ...
          <span class="font-bold text-slate-800 dark:text-slate-100 text-sm tracking-tight">
            LLM Companion
          </span>
        </div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (settings) {
                  setSettings({
                    ...settings,
                    lastSelectedPromptId: newPromptId,
                  });
                }
              }}
        ...
        <div class="p-3 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 shrink-0">
          <div class="flex items-end gap-2">
            <textarea
              value={followUpText}
              onInput={(e) =>
                setFollowUpText((e.target as HTMLTextAreaElement).value)
              }
