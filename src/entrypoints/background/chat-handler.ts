...
export async function handleExecutePrompt(
  ...
  if (formattedPageContext && contextLimit) {
    ...
    const currentChars =
      settings.systemPrompt.length + formattedPageContext.length;
    const maxContextChars = Math.max(approxCharLimit - reserveChars, 0);

    if (approxCharLimit > 0 && currentChars > maxContextChars) {
      const truncated = formattedPageContext.slice(
        0,
        Math.max(maxContextChars - settings.systemPrompt.length, 0),
      );
  ...
  if (systemContent.length > 50000) {
    console.warn(
      "[Chat Handler] Large context detected:",
      systemContent.length,
      "characters",
    );
    console.warn(
      "[Chat Handler] Some models may truncate or fail with this size",
    );
  }
  ...
  console.debug(
    "[Chat Handler] Total messages in conversation:",
    messages.length,
  );
  ...
  try {
    ...
    console.debug(
      "[Chat Handler] Assistant message will be at index:",
    ...
    console.debug(
      "[Chat Handler] Final response length:",
      fullResponse.length,
      "characters",
    );
    ...
    try {
      ...
      if (
        ...
        errorMessages[errorMessages.length - 1].role === "assistant"
      ) {
      ...
      console.error(
        "[Chat Handler] Failed to update session after error:",
        storageError,
      );
      ...
      try {
        ...
        console.error(
          "[Chat Handler] Fatal: Could not update any session state:",
          finalError,
        );
      }
    }
