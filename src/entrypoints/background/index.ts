...
async function handleContextMenuClick(
  ...
  if (
    ...
    !info.menuItemId.startsWith(CONTEXT_ITEM_PREFIX)
  )
    return;
  const promptId = info.menuItemId.replace(CONTEXT_ITEM_PREFIX, "");
  const selection = info.selectionText
    ? truncateSelection(info.selectionText)
    : "";
  if (!selection) {
  ...
  try {
    ...
    console.error(
      "[Background] Failed to send EXECUTE_SELECTED_TEXT_PROMPT:",
      e,
    );
  }
}
...
export default defineBackground(() => {
  ...
  settingsStorage.watch((newSettings) => {
    ...
    if (currentSignature !== lastProvidersSignature) {
      ...
      refreshDebounceTimer = setTimeout(() => {
        console.debug(
          "[Background] Provider settings changed, refreshing models...",
        );
        refreshDiscoveredModels();
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    try {
      ...
      console.warn(
        "[Background] Could not send response for unknown message:",
        e,
      );
    }
    return false;
