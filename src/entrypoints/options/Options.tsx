...
export default function Options() {
  ...
  const getProviderStatus = (
    ...
    const hasModels = settings.discoveredModels?.some(
  ...
  const updateProvider = (id: keyof AppSettings["providers"], updates: any) => {
    ...
    const isEnablingOnly =
      updates.enabled === true && Object.keys(updates).length === 1;
      ...
      <aside class="w-44 flex flex-col shrink-0">
        <nav class="flex flex-col gap-4">
          {tabs.map((tab) => (
            ...
            >
              <tab.icon
                class={`w-4 h-4 shrink-0 ${activeTab === tab.id ? "text-indigo-600" : ""}`}
              />
              <span>{tab.label}</span>
            </button>
          ))}
                    ...
                    {localPrompts.map((prompt) => (
                      <tr key={prompt.id}>
                        ...
                        <td class="align-top">
                          ...
                          >
                            <option value={PromptType.WITH_WEBPAGE}>
                              With Webpage
                            </option>
                            <option value={PromptType.FREE_TEXT}>
                              Free Text
                            </option>
                            <option value={PromptType.SELECTED_TEXT}>
                              Selected Text
                            </option>
                          </select>
                        </td>
                            ...
                            onInput={(e) => {
                              const newPrompts = localPrompts.map((p) =>
                                p.id === prompt.id
                                  ? {
                                      ...p,
                                      text: (e.target as HTMLTextAreaElement)
                                        .value,
                                    }
                                  : p,
                              );
                        ...
                        <td class="text-center align-top">
                          <div class="flex items-start justify-center pt-2.5">
                            <button
                              onClick={() => {
                                if (!prompt.isDefault) {
                                  const newPrompts = localPrompts.filter(
                                    (p) => p.id !== prompt.id,
                                  );
              ...
              <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 shadow-sm">
                <div class="text-center space-y-1">
                  ...
                  <p class="text-sm text-slate-600 dark:text-slate-300">
                    An Open Source Extension harnessing local and cloud LLM
                    power in the browsers
                  </p>
                </div>
                      ...
                      {[
                        ...
                        >
                          <div class="flex-1 min-w-0">
                            ...
                            >
                              <span class="font-bold">{crypto.symbol}:</span>{" "}
                              {crypto.addr}
                            </a>
                          </div>
                          <button
