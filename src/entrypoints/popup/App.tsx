...
export default function App() {
    ...
    // Auto-select the last used model if available, otherwise the first model
    useEffect(() => {
        if (!settings || !models.length) return;
        if (!settings.selectedModelId || !models.some(m => `${m.providerId}:${m.id}` === settings.selectedModelId)) {
            // Pick the first model if last used is not available
            setSettings({
                ...settings,
    ...
    return (
        <div class="flex flex-col w-[530px] min-w-[460px] max-w-[95vw] h-auto min-h-0 overflow-hidden bg-slate-100 dark:bg-slate-950">
            {/* Header */}
