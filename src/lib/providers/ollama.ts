...
export class OllamaProvider extends BaseProvider {
    ...
    try {
      ...
      for (const m of data.models || []) {
        try {
          ...
          console.warn(
            `[Ollama] Could not fetch details for ${m.name}:`,
            showError,
          );
  ...
  async *stream(
    ...
    messages.forEach((msg, idx) => {
      console.debug(
        `[Ollama] Message ${idx} - Role: ${msg.role}, Length: ${msg.content.length} chars`,
      );
      if (msg.content.length > 1000) {
        console.debug(
          `[Ollama] Message ${idx} preview:`,
